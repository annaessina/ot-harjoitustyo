...
class CreateNew():
    ...
    def _create(self):
        ...
        create_label = ttk.Label(master=self._frame, text="Create new account")
        create_label.grid(row=1, columnspan=2,
                          sticky=constants.W, padx=5, pady=5)

        username_label = ttk.Label(
            master=self._frame, text="Enter your name: ")
        self._username_entry = ttk.Entry(master=self._frame)
        username_label.grid(row=2, column=0, padx=5, pady=5)
        self._username_entry.grid(row=2, column=1, sticky=(
            constants.E, constants.W), padx=5, pady=5)

        password_label = ttk.Label(master=self._frame, text="Enter your password: ")
        ...
        create = ttk.Button(master=self._frame,
                            text="Create new account", command=self._create_process)
    ...
    def _create_process(self):
        ...
        if len(username) == 0:
            self._show_error("Enter your name")
            return
        if len(password) == 0:
